  ...
  class LifecycleStatus
    ...
    class << self
      def snapshot(cluster)
        ...
        nodes = kubectl_json(cluster, %w[get nodes -o json])
        ...
        worker_groups = Array(cluster.node_groups).map do |ref|
          ...
          status = group_status(cluster, group, node_rows, 'machinedeployment', group.uuid)
      ...
      def aggregate(groups)
        keys = %i[
          desired provider_desired provider_replicas provider_ready
          provider_available provider_updated created joined ready
        ]
        keys.to_h do |key|
