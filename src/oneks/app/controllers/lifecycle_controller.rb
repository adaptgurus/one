...
module OneKS
    ...
    # Helper methods shared by LayerSentry lifecycle routes.
    module LifecycleControllerHelpers
        ...
        def lifecycle_worker_group(cluster, group_id)
            ref = Array(cluster.node_groups).find do |entry|
                entry[:id].to_i == group_id.to_i
            end
            return OpenNebula::Error.new(
    ...
    module LifecycleController
        ...
        def self.registered(app)
            app.helpers LifecycleControllerHelpers
            ...
            app.post '/clusters/:id/control-plane/scale' do
                ...
                unless target.is_a?(Integer) &&
                       (target == 1 || (target.between?(3, ControlPlane::MAX_REPLICAS) && target.odd?))
                    return internal_error(
                ...
                rc = cluster.scale_group(
                    cluster.control_plane[:id], target, :actor => @username
                )
                return internal_error(
            ...
            # Stage 1 of a Kubernetes version upgrade. The target version is
            # persisted on the cluster and only RKE2ControlPlane is reconciled.
            # LayerSentry waits for Ready/etcd/version convergence before workers.
            app.post '/clusters/:id/control-plane/upgrade' do
                ...
                rc = cluster.upgrade_group(
                    cluster.control_plane[:id], :actor => @username
                )
                return internal_error(
                    rc.message, one_error_to_http(rc.errno)
                ) if OpenNebula.is_error?(rc)
            ...
            # Stage 2 runs per worker group after the control plane has fully
                ...
                body process_response(
                    {
                        :cluster_id => cluster.id,
                        :nodegroup_id => group.id,
                        :shape => group.user_inputs_values.slice(
                            :cpu, :vcpu, :memory, :disk_size
                        ),
            ...
            app.post '/clusters/:id/nodegroups/:nodegroup_id/autoscaling' do
                ...
                unless min.is_a?(Integer) && max.is_a?(Integer)
                    return internal_error(
                        'Fields min and max must be integers',
                        ODS::ResponseHelper::VALIDATION_EC
                    )
                end
                ...
                rc = group.configure_autoscaling(
                    :enabled => enabled, :min => min, :max => max
                )
                return internal_error(
                    rc.message, one_error_to_http(rc.errno)
                ) if OpenNebula.is_error?(rc)
