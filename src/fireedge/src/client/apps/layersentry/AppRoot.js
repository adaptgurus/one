...
const isDisabledLayoutRoute = (pathname, routes = []) => {
  const matches = ({ path, disableLayout } = {}) =>
...
/** @returns {ReactElement} LayerSentry application root. */
const LayerSentryApp = () => {
  ...
  useEffect(() => changeAppTitle(APP_NAME), [])
  useEffect(() => {
...
LayerSentryApp.displayName = 'LayerSentryApp'
export default LayerSentryApp
