...
test('server independently validates and reconciles the native network', () => {
  ...
  assert.match(source, /serviceUnavailable/)
  assert.match(source, /authoritative state is unknown/)
  assert.match(source, /template\.LAYERSENTRY_TIER/)
  assert.match(source, /template\.NETWORK_ADDRESS/)
