...
const run = async () => {
  ...
  assert.equal(await page.locator('#draft-input').inputValue(), 'unsaved customer value')
  ...
  for (let index = 0; index < 8; index += 1) {
    ...
    if ((await page.evaluate(() => document.activeElement?.id)) === 'draft-input') break
  }
  ...
  const evidence = await page.evaluate(() => window.__LAYERSENTRY_BROWSER_EVIDENCE__)
  ...
  assert.equal(evidence.gpuEvidence.pci[0].PROFILE, '1146 (NVIDIA L40S-2B)')
  ...
  await page.screenshot({ path: 'tests/layersentry/browser/browser-smoke.png', fullPage: true })
  await browser.close()
}
